  ...
  async function handleStart() {
    ...
    try {
      const resp = await fetch(`/api/projects/${projectName}/agents/${issueId}/services`, {
        ...
        body: JSON.stringify({ enabled: true, mode: "host" }),
      });
      if (!resp.ok) {
        const data = await resp.json();
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="flex items-center gap-2 text-xs">
        ...
        {/* Dir indicator when active */}
        {enabled && !isStopped && targetInfo && (
          <span className="text-[10px] text-muted-foreground font-mono truncate max-w-[200px]" title={targetInfo}>
            [{targetInfo.split(/[\\/]/).slice(-2).join("/")}]
          </span>
        )}
        ...
        {/* Start — show when fully stopped / never started */}
        {!enabled && !toggling && (
