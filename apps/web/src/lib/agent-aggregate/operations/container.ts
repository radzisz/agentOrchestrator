...
/** Check container status. Returns the detected status without mutating state. */
export async function checkContainer(
  agent: store.AgentData,
): Promise<AgentState["container"]> {
...
export async function createContainer(
  ...
  const driver = getProviderDriver(providerCfg);
...
export async function ensureContainerRunning(
  ...
  const status = await checkContainer(agent);
...
export async function stopContainer(
  ...
  await cmd.run(`docker stop "${agent.containerName}"`, { source: "stopContainer", timeout: 30_000 }).catch(() => {});
