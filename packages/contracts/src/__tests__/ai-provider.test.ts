...
import { BaseAIProvider, type AIProviderDriver, type ProviderTypeSchema } from "../ai-provider.js";
...
class MockAIProvider extends BaseAIProvider {
  ...
  createDriver(config: Record<string, string>): AIProviderDriver {
    ...
    return {
      ...
      buildLaunchCommand(prompt: string) { return `mock-run --model ${model} "${prompt}"`; },
      buildEnvVars() { return [`MODEL=${model}`]; },
      filterOutput(raw: string) { return raw.trim(); },
    };
  }
}
