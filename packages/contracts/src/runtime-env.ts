...
export interface RtenvProvisionResult {
  ...
  deployIds?: Array<{ name: string; id: string }>;
  error?: string;
}
...
export interface RtenvStatusResult {
  ...
  url?: string;
  error?: string;
  deploys?: Array<{ name: string; state: string; url?: string; error?: string }>;
}
...
export abstract class BaseRuntimeEnv {
  ...
  async triggerDeploy?(
    ...
    projectConfig: Record<string, string>,
    branch: string,
  ): Promise<Array<{ name: string; deployId: string }>>;
}
