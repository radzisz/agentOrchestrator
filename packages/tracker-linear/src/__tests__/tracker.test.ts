...
function linearApiResponse(data: any) {
  return { ok: true, json: () => Promise.resolve({ data }) };
}

function makeLinearIssue(overrides: any = {}) {
  return {
    id: "uuid-1",
    identifier: "UKR-101",
    title: "Fix login bug",
    ...
    priority: 2,
    url: "https://linear.app/team/UKR-101",
    state: { name: "Todo" },
    labels: { nodes: [{ id: "l1", name: "agent" }] },
    creator: { id: "u1", name: "John" },
    assignee: { id: "u2", name: "Agent" },
    comments: { nodes: [
...
describe("LinearTracker", () => {
  ...
  describe("transitionTo", () => {
    it("no-op without apiKey", async () => {
      await tracker.transitionTo!({}, makeLinearIssue() as any, "done");
      expect(mockFetch).not.toHaveBeenCalled();
    });
    ...
    it("resolves state ID and updates issue", async () => {
      ...
      const issue = { externalId: "uuid-1" } as any;
  ...
  describe("getComments", () => {
    it("returns comments from _raw LinearIssue", async () => {
      ...
      const issue = { _raw: rawIssue } as any;
  ...
  describe("hasLabel", () => {
    it("checks labels array", () => {
      expect(tracker.hasLabel!({ labels: ["agent", "preview"] } as any, "agent")).toBe(true);
      expect(tracker.hasLabel!({ labels: ["agent"] } as any, "missing")).toBe(false);
    });
  });

  describe("reassignOnDone", () => {
    it("no-op when not in assignee mode", async () => {
      await tracker.reassignOnDone!({ apiKey: "key", detectionMode: "label" }, makeLinearIssue() as any);
      expect(mockFetch).not.toHaveBeenCalled();
    });
    ...
    it("reassigns to creator in assignee mode", async () => {
      ...
      await tracker.reassignOnDone!(
        { apiKey: "key", detectionMode: "assignee", reassignOnDone: "true" },
        issue,
      );
      expect(mockFetch).toHaveBeenCalledOnce();
    });
    ...
    it("skips if creator is already assignee", async () => {
      ...
      await tracker.reassignOnDone!(
        { apiKey: "key", detectionMode: "assignee" },
        issue,
      );
      expect(mockFetch).not.toHaveBeenCalled();
    });
