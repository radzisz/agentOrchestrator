...
export function LinearProjectConfigPanel({
  overrideFields,
  overrides,
  resolvedConfig,
  projectName,
  ...
  useEffect(() => {
    if (detectionMode !== "assignee") return;
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        ...
        {loadingTeams ? (
          ...
          <select
            ...
            value={currentTeamId}
            onChange={(e) => {
              const team = teams.find((t: LinearTeam) => t.id === e.target.value);
              setField("teamId", e.target.value);
              setField("teamKey", team?.key || "");
            }}
          >
          ...
          <input
            className="w-full px-2 py-1 text-sm border rounded bg-background"
            value={overrides.teamId || ""}
            onChange={(e) => setField("teamId", e.target.value)}
            placeholder="Team ID (could not load teams)"
          />
        )}
      </div>
        ...
        <div className="flex gap-4">
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              type="radio"
              name="detectionMode"
              checked={detectionMode === "assignee"}
              onChange={() => setField("detectionMode", "assignee")}
              className="accent-primary"
            />
      ...
      {detectionMode === "label" && (
        <OverrideField
          field={{ key: "label", label: "Detection Label", type: "string", default: "agent", description: "Label to detect agent issues" }}
      ...
      {detectionMode === "assignee" && (
        <div>
          ...
          {loadingMembers ? (
            ...
            <select
              className="w-full px-2 py-1 text-sm border rounded bg-background"
              value={overrides.assigneeId || resolvedConfig.assigneeId || ""}
              onChange={(e) => {
                const member = members.find((m: LinearMember) => m.id === e.target.value);
                setField("assigneeId", e.target.value);
                setField("assigneeName", member?.displayName || member?.name || "");
              }}
            >
              <option value="">Select team member...</option>
              {members.map((m: LinearMember) => (
            ...
            <input
              className="w-full px-2 py-1 text-sm border rounded bg-background"
              value={overrides.assigneeId || ""}
              onChange={(e) => setField("assigneeId", e.target.value)}
              placeholder="Assignee ID (could not load members)"
            />
          )}
          {overrides.assigneeName && (
            <p className="text-[11px] text-muted-foreground mt-1">Selected: {overrides.assigneeName}</p>
          )}
          ...
          <div className="mt-2">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                checked={(overrides.reassignOnDone || resolvedConfig.reassignOnDone || "true") === "true"}
                onChange={(e) => setField("reassignOnDone", e.target.checked ? "true" : "false")}
                className="accent-primary"
              />
          ...
          <div className="space-y-2">
            ...
            {selectedProjectIds.length > 0 && (
              <div className="space-y-1">
                {selectedProjectIds.map((pid: string) => {
                  const proj = linearProjects.find((p: LinearProject) => p.id === pid);
                  return (
                    <div key={pid} className="flex items-center gap-2 text-sm bg-muted/30 rounded px-2 py-1">
            ...
            {(() => {
              const available = linearProjects.filter((p: LinearProject) => !selectedProjectIds.includes(p.id));
            ...
            {selectedProjectIds.length === 0 && (
              <p className="text-[11px] text-muted-foreground">No projects selected — all issues will be tracked</p>
            )}
          </div>
        ) : (
          <p className="text-xs text-muted-foreground">No Linear projects found</p>
        )}
      </div>
    </div>
  );
