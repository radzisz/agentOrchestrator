...
function sentryResponse(data, status = 200) {
...
function makeSentryIssue(overrides = {}) {
    return Object.assign({ id: "12345", shortId: "UKRYTE-SKARBY-PANEL-39", title: "TypeError: Cannot read property 'map'", culprit: "components/UserList.tsx", permalink: "https://sentry.io/issues/12345/", level: "error", count: "42", userCount: 15, firstSeen: "2026-01-01T00:00:00Z", lastSeen: "2026-03-01T00:00:00Z", metadata: { value: "Cannot read property 'map' of undefined" }, project: { slug: "ukryte-skarby-panel", name: "USP" }, status: "unresolved", isUnhandled: true }, overrides);
}
describe("SentryTracker", () => {
    ...
    describe("transitionTo", () => {
        it("resolves to Sentry status and calls API", async () => {
            mockFetch.mockResolvedValue(sentryResponse({}));
            const issue = { externalId: "12345" };
            await tracker.transitionTo({ authToken: "tok" }, issue, "done");
            expect(mockFetch).toHaveBeenCalledWith("https://sentry.io/api/0/issues/12345/", expect.objectContaining({
                method: "PUT",
        ...
        it("maps phases to sentry statuses", async () => {
            ...
            for (const [phase, sentryStatus] of mapping) {
                mockFetch.mockResolvedValue(sentryResponse({}));
                await tracker.transitionTo({ authToken: "tok" }, { externalId: "1" }, phase);
        ...
        it("no-op for in_progress/in_review phases", async () => {
            await tracker.transitionTo({ authToken: "tok" }, { externalId: "1" }, "in_progress");
